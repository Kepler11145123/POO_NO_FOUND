...
dificultad = int(input("ingrese el numero de intentos por partida que desea tener: "))

# Crear una matriz de 5x5 llena de guiones
matriz = [['-' for _ in range(5)] for _ in range(5)]

# Lista de elementos que quieres colocar
elementos = ['T', 'X', 'X', 'X']

# Obtener todas las posibles coordenadas (fila, columna)
coordenadas_disponibles = [(f, c) for f in range(5) for c in range(5)]

# Seleccionar 4 coordenadas aleatorias (una para cada elemento)
coordenadas_aleatorias = random.sample(coordenadas_disponibles, 4)
